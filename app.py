...
def validate_ticker(ticker):
    """Validate ticker symbol with error handling."""
    try:
        stock = yf.Ticker(ticker)
        info = stock.info
        
        if not info or 'regularMarketPrice' not in info or info['regularMarketPrice'] is None:
            return False, "Invalid ticker symbol or no data available"
            
        if 'symbol' not in info or info['symbol'] != ticker.upper():
            return False, "Ticker symbol mismatch or not found"
